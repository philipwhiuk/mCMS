...
class Image_File {
	...
	public function resize($height, $width){
		...
		$db = MCMS::Get_Instance()->Storage();

		$query = $db->Insert()->From('image_file')
			->set(array(
				'image' => array('u', $image),
	...
	public static function Get_By_Image($image){
		...
		$query = MCMS::Get_Instance()->Storage()->Get()->From('image_file')->where('=',array( 
			array('col', 'image'),
			array('u', $image)
		));
	...
	public static function Get_One($operator, $operand){
		...
		$query = MCMS::Get_Instance()->database()->Select()->table('image_file')->where($operator, $operand)->limit(1);
