<?php
abstract class Simm_Page_Main_Spec extends Simm_Page_Main {
	public static function Load($parent) {
		$ID = $parent->resource()->get_argument();
		if(is_numeric($ID)) {
			try {
				$ID = $parent->resource()->get_argument();					
				$spec = Simm_Specification::Get_By_ID($ID);
				$parent->resource()->consume_argument();
				$mode = $parent->resource()->get_argument();
				switch($mode) {
					case 'edit':
						$parent->resource()->get_module()->file('page_main/spec/edit');
						$parent->resource()->consume_argument();
						return new Simm_Page_Main_Spec_Edit($parent, $spec);		
						break;
					case 'view':
					default:
						$parent->resource()->get_module()->file('page_main/spec/view');
						$parent->resource()->consume_argument();
						return new Simm_Page_Main_Spec_View($parent, $spec);		
						break;
				}
	...
	protected function check($mode){
		...
		$perms = Permission::Check(array('spec',$id), array('view','edit','add','delete','list','admin'),$mode);
		...
		foreach($perms as $k => $allowed){
			if($allowed && in_array($k, array('view','edit','delete'))){
				...
				$res = Resource::Get_By_Argument(Module::Get('simm'),'spec'.$id .'/' . $k);
				$this->modes[$k] = array(
