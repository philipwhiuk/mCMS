...
class Local_Login extends Login {
	...
	public function salt(){
		try {
			return System::Get_Instance()->site()->get('local_salt');
		} catch(Exception $e){
			try {
				return System::Get_Instance()->config()->get('local_salt');
			} catch(Exception $f){
	...
	public function display(){
		$template = System::Get_Instance()->output()->start(array('local','login'));
