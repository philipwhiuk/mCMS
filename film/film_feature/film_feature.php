...
class Film_Feature {
	...
	public function Add($data){
		$database = MCMS::Get_Instance()->database();
	...
	public function update($data){
		$query = MCMS::Get_Instance()->database()->Update()->table('content')
			->set(array(
					'title' => array('s', $data['title']),
					'body' => array('s', $data['body'])
			))
	...
	public static function Get_All(){
		$query = MCMS::Get_Instance()->Storage()->Get()->From('film_feature');
	...
	public static function Get_By_ID($arg){
		if(is_array($arg)) {
			if(sizeof($arg) > 0) {
				...
				$query = MCMS::Get_Instance()->Storage()->Get()->From('film_feature')->where('in',$operand);
				$result = $query->execute();
				$return = array();
				while($row = $result->fetch_object('Film_Feature')){
	...
	public static function Get_One($operator, $operand){
		...
		$query = MCMS::Get_Instance()->Storage()->Get()->From('film_feature')->where($operator, $operand)->limit(1);
	...
	public static function Get_Category_Next($category) {
		...
		foreach($coming_soon as $showing) {
			...
			$query = MCMS::Get_Instance()->Storage()->Get()->From('film_feature')->where('AND',$operand)->limit(1);
			$result = $query->execute();
			if($result->num_rows > 0) {
				return $result->fetch_object('Film_Feature');
			}
	...
	public static function Count_All(){
		$query = MCMS::Get_Instance()->Storage()->Count()->From('film_feature');
