...
class Film_Feature_Film_Certificate extends Film_Certificate {
	...
	public static function Get_By_ID($id) {
		...
		$query = MCMS::Get_Instance()->database()->Select()->table('film_certificate')->where($operator, $operand)->limit(1);
	...
	public static function Get_By_Film($film) {
		...
		$query = MCMS::Get_Instance()->database()->Select()->table('film_trailer')->where($operator, $operand);
		$result = $query->execute();
		$return = array();
